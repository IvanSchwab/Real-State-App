...
const Nav = ({ openNav }: Props) => {
    ...
    return (
        <div
            className={`fixed h-[10vh] z-[50] w-full transition-all duration-500 ease-in-out ${navBg ? "bg-custom-green" : "bg-transparent"
                }`}

        >
            <div className="flex left-3 items-center h-[80%] mt-2 justify-between w-[95%] sm:w-[90%] md:w-[85%] xl:w-[80%] mx-auto relative">
                <div
                    className={`bg-[#84ac84]  opacity-65 p-2 sm:ml-2 transition-opacity duration-500 ${navBg ? "opacity-90" : "opacity-70"
                        } rounded-md max-w-xs shadow-md sm:ml-[-50px] md:ml-[-20px]`}
                >
                    <img
